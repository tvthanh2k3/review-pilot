...
Single Next.js 16 app (App Router) backed by Supabase and wired to the Google Places API and OpenAI.

| Layer | Technologies |
|---|---|
| Frontend | Next.js 16, React 19, TypeScript, Tailwind CSS 4 |
| Backend | Next.js Route Handlers, Supabase (PostgreSQL + Auth), OpenAI SDK |
| Integrations | Google Places API (review fetch), OpenAI (reply generation) |
...
- AI reply generation — context-aware draft reply via OpenAI
...
`GOOGLE_PLACES_API_KEY` and `OPENAI_API_KEY` are secret server-side keys. Google Places fetches and OpenAI calls must only happen inside Route Handlers or Server Components — never in Client Components, and never prefixed with `NEXT_PUBLIC_`.

**Route Handlers**
API routes live in `src/app/api/`. Use Route Handlers (`route.ts`) for Google Places fetches and OpenAI calls. Prefer Server Actions for Supabase mutations triggered from forms.
